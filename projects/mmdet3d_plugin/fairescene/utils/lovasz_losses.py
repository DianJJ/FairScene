...
def lovasz_softmax(probas, labels, classes='present', valid_mask = None, per_image=False, ignore=None):
    """
    Multi-class Lovasz-Softmax loss
      probas: [B, C, H, W] Variable, class probabilities at each prediction (between 0 and 1).
              Interpreted as binary (sigmoid) output with outputs of size [B, H, W].
    ...
    """
    if per_image:
        loss = mean(lovasz_softmax_flat(*flatten_probas(prob.unsqueeze(0), lab.unsqueeze(0), valid_mask), classes=classes)
                          for prob, lab in zip(probas, labels))
    else:   # <-
        loss = lovasz_softmax_flat(*flatten_probas(probas, labels, valid_mask), classes=classes)
    return loss
...
def flatten_probas(probas, labels, valid_mask = None):
    """
    Flattens predictions in the batch
    """
    if probas.dim() == 3:
        ...
        #3D segmentation
        B, L, H, W,C = probas.size()
        probas = probas.contiguous().view(B, L, H*W, C)
        labels = labels.contiguous().view(B, L, H*W, C)
        #probas = probas.contiguous().view(B, C, L, H * W)
    if probas.dim() == 4:
        B, H, W, C = probas.size()
        probas = probas.contiguous().view(-1, C)  # B * H * W, C = P, C
        labels = labels.contiguous().view(-1, C)
    else:
        probas = probas.contiguous()
    ...
    if valid_mask is None:
        return probas, labels
    # valid = (labels != ignore)
    vprobas = probas[valid_mask[0]]
    vlabels = labels[valid_mask[0]]
    return vprobas, vlabels
