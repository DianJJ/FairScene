...
class FLoSP(nn.Module):
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        num_mix = len(projected_pix[0])
        bs, num_cam, _, _, _ = mlvl_feats[0].shape
        device = mlvl_feats[0].device

        projected_pix_ori=projected_pix
        fov_mask_ori=fov_mask
        ...
        for i in range(num_mix):
            #（1,2,26144,2）
            projected_pix = torch.from_numpy(projected_pix_ori[:,i:i+1,:,:]).flatten(0, 1).long().to(device)
            #(1,26144,2)
            #(1,2,26144)
            fov_mask = torch.from_numpy(fov_mask_ori[:,i:i+1,:]).flatten(0, 1).to(device)
