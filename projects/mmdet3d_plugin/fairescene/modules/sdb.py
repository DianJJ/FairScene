...
class ASPP(nn.Module):
    def __init__(self, channel, groups=20):
        super(ASPP, self).__init__()
        self.feature = channel
        c = channel // 2
        self.reduce = nn.Conv3d(channel, c, kernel_size=1, groups=groups)
        self.aspp_1 = nn.Conv3d(c, c, kernel_size=3, padding=1, dilation=1, groups=groups)
        self.aspp_2 = nn.Conv3d(c, c, kernel_size=3, padding=2, dilation=2, groups=groups)
        self.aspp_4 = nn.Conv3d(c, c, kernel_size=3, padding=4, dilation=4, groups=groups)
        self.fusion = nn.Conv3d(c * 3, channel, kernel_size=1, groups=groups)

    def forward(self, x3d):
        # x3d shape: [batch_size, C, D, H, W]
        x = self.reduce(x3d)
        x_1 = self.aspp_1(x)
        x_2 = self.aspp_2(x)
        x_3 = self.aspp_4(x)
...
#####################group conv version
class MPAC(nn.Module):
    def __init__(self, channel, kernel=(3, 5, 7), dilation=(1, 1, 1), residual=False, groups=20):
        super().__init__()
        self.channel = channel
        self.residual = residual
        self.n = len(kernel)  # number of kernels
        self.groups = groups
        self.conv_mx = nn.Conv3d(channel, 3 * self.n, (1, 1, 1), stride=1, padding=0, bias=False)
        self.softmax = nn.Softmax(dim=2)  # Applies the Softmax function along the scale dimension
        ...
        for _idx in range(self.n):
            ...
            d = dilation[_idx]
            p = k // 2 * d
            self.conv_1x1xk.append(nn.Conv3d(c, c, (1, 1, k), stride=1, padding=(0, 0, p), bias=True, dilation=(1, 1, d), groups=groups))
            self.conv_1xkx1.append(nn.Conv3d(c, c, (1, k, 1), stride=1, padding=(0, p, 0), bias=True, dilation=(1, d, 1), groups=groups))
            self.conv_kx1x1.append(nn.Conv3d(c, c, (k, 1, 1), stride=1, padding=(p, 0, 0), bias=True, dilation=(d, 1, 1), groups=groups))

    def forward(self, x):
        mx = self.conv_mx(x)  # (BS, 3n, D, H, W)
        _bs, _, _d, _h, _w = mx.size()
        mx = mx.view(_bs, 3, -1, _d, _h, _w).contiguous()  # (BS, 3, n, D, H, W)
...
class SDB(nn.Module):
    def __init__(self, channel, out_channel, depth=2, version='v1', groups=20): ##depth original = 3
        super(SDB, self).__init__()

        c = out_channel
        self.groups = groups
        #c = 640
        self.conv_in = nn.Conv3d(channel, c, kernel_size=3, padding=1, groups=groups)

        basic_block = MPAC if version == 'v1' else MPACv2
        blocks = [basic_block(c, residual=True, groups=groups) for _ in range(depth)]
        self.diffusion = nn.Sequential(*blocks)
        self.aspp = ASPP(c, groups=groups)
